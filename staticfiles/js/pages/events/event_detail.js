document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCartDisplay() {
        ...
        document.querySelectorAll('.ticket-card').forEach(card => {
            ...
            if (qty > 0) {
                ...
                itemsHtml += '<div class="cart-item" role="button" tabindex="0" title="Нажмите, чтобы посмотреть полностью">' + itemLabel + '</div>';
        ...
        if (cartSection) {
            if (totalItems > 0) {
                ...
                // Помечаем сокращённые названия корзины сразу после рендера
                cartItems.querySelectorAll('.cart-item').forEach(function(el) {
    ...
    // === СОКРАЩЁННЫЕ ТЕКСТЫ БИЛЕТОВ: одна строка, клик — полный текст ===
    // Охватывает: описания билетов на странице (.ticket-desc),
    // названия билетов в модалке покупки (.ticket-name-text)
    // и названия в корзине модалки (.cart-item, создаются динамически —
    // поэтому обработчики через делегирование, а не напрямую на элементах)
    (function initTicketTextTooltips() {
        var selector = '.ticket-desc, .ticket-name-text, .cart-item';

        // Помечаем элементы, чей текст не влазит (по ширине для nowrap-строк,
        // по высоте — на случай line-clamp)
        ...
        // Модалка покупки изначально скрыта (нулевые размеры) —
        // пересчитываем после её показа
        var buyModalEl = document.getElementById('buyTicketModal');
        if (buyModalEl) {
            buyModalEl.addEventListener('shown.bs.modal', function() {
                markTruncated(buyModalEl);
            });
        }
        ...
        // Делегирование: работает и для динамически добавленных .cart-item
        document.addEventListener('click', function(e) {
            var el = e.target.closest(selector);
            if (el && el.classList.contains('is-truncated')) {
                e.stopPropagation();
        ...
        document.addEventListener('keydown', function(e) {
            ...
            // Доступность с клавиатуры: Enter/Пробел на сфокусированном элементе
            if (e.key === 'Enter' || e.key === ' ') {
                var el = document.activeElement;
                if (el && el.matches && el.matches(selector) && el.classList.contains('is-truncated')) {
                    e.preventDefault();
                    if (activeEl === el && tooltip) {
                        hideTooltip();
                    } else {
        ...
        // При ресайзе пересчитываем, что не влазит, и прячем тултип
